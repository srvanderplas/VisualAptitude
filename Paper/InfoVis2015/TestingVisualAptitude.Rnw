...
ggbiplot <- function(pca, choices, segColor="black") {
  ...
  v <- data.frame(sweep(v, MARGIN=2, 1/sqrt(rowSums(v^2))[choices], FUN="*"))
  v$label <- row.names(v)
...
ggbiplot(pca, choices=2:3, segColor=c("red", rep("black", 4))) + xlim(c(-2.6,2.6)) +  ylim(c(-2.6,2.6))
ggbiplot(pca, choices=4:5, segColor=c("#FF3333", rep("black", 4)))
@
