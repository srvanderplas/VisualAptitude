...
<<ggbiplots-pca5, fig.cap="", fig.width=4.5, fig.height=4.5, warning=F, message=F, out.width='.45\\linewidth', comment="|", fig.cap="Biplots of principal components 2-5 with observations. The lineup task appears to be most similar to the figure classification task, based on the plot of PC2 vs. PC3.  \\label{fig:biplots5}", echo=F, fig.env="figure">>=
ggbiplot <- function(pca, choices) {
  u <- pca$x[,choices]
  ...
  qplot(u[,1], u[,2], data=u, colour=I("grey50"), shape=I(1), size=I(3)) + theme_bw() + theme(aspect.ratio=1) + coord_equal() + 
  geom_segment(aes(x=0, y=0, xend=v[,1], yend=v[,2]), data=v, arrow=arrow(length = unit(0.3, "cm"))) + 
    xlab(sprintf("PC%d",choices[1])) + ylab(sprintf("PC%d",choices[2])) +
    geom_text(aes(x=v[,1], y=v[,2], label=label, hjust=0.5*(0.5-sign(v[,1])), vjust=0.5-sign(v[,2])), data=v)
}

ggbiplot(pca, choices=2:3) + xlim(c(-2.6,2.6)) +  ylim(c(-2.6,2.6))
ggbiplot(pca, choices=3:4)
@
