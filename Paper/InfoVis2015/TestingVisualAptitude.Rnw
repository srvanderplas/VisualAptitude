...
%% Uncomment below to include a teaser figure.
   \teaser{
      ...
      \begin{minipage}[c]{0.4\textwidth}
         \subfloat[][``Which of these plots is the most different from the others?"]{\label{fig:lineup} \includegraphics[height=\textwidth]{lineup.png}}
        %
        \end{minipage}
        \begin{minipage}[c]{.4\textwidth}
         \subfloat[][Biplots]{\label{fig:biplots}
          \includegraphics[height=.8\textwidth]{Figure/fig-biplots-pca5-1}\hfill\newline
          \includegraphics[height=.8\textwidth]{Figure/fig-biplots-pca5-2}
         }
          \end{minipage}
    \caption{On the left a sample lineup of boxplots is shown. Participants are asked to identify the most different plot.  %Participants are instructed to choose the plot which appears most different from the others. 
    Here, panel 4 is the target plot, because the two groups have a large difference in medians.\\
...
ggbiplot <- function(pca, choices, segColor="black") {
  ...
  geom_segment(aes(x=0, y=0, xend=v[,1], yend=v[,2], colour=segColor), data=v, arrow=arrow(length = unit(0.3, "cm"))) + 
    ...
    geom_text(aes(x=v[,1], y=v[,2], label=label, colour=segColor, 
                  hjust=0.15*(0.5-sign(v[,1])), vjust=0.5-sign(v[,2])), data=v) +
...
<<biplot-pca4, fig.cap="", fig.width=4.5, fig.height=4.5, warning=F, message=F, out.width='.45\\linewidth', comment="|", fig.cap="Biplots of principal components 1-4 with observations (same as figure 1(b)). Principal component analysis was performed on the four cognitive tests used to understand the association between the cognitive skills required for these tests and the skills required for the lineup protocol.  \\label{fig:biplots4}", echo=F, fig.env="figure">>=
#par(mar=c(4,4,.1,.1),cex.lab=.95,cex.axis=.9,mgp=c(2,.7,0),tcl=-.3)
dimnames(pca2$rotation)[[1]] <- c("card rotation", "figure classification", "paper folding", "visual search")

ggbiplot(pca2, choices=1:2) + xlim(c(-2.5,2.5)) +  ylim(c(-2.5,2.5))
ggbiplot(pca2, choices=3:4) + xlim(c(-3,3)) + ylim(c(-2.5,2.5))
@
...
% <<biplots-pca5, fig.cap="", fig.width=4.5, fig.height=4.5, warning=F, message=F, out.width='.45\\linewidth', comment="|", fig.cap="Biplots of principal components 2-5 with observations. The lineup task appears to be most similar to the figure classification task, based on the plot of PC2 vs. PC3.  \\label{fig:biplots5}", echo=F, fig.env="figure">>=
% par(mar=c(4,4,.1,.1),cex.lab=.95,cex.axis=.9,mgp=c(2,.7,0),tcl=-.3)
...
dimnames(pca$rotation)[[1]] <- c("lineup", "card rotation", "figure classification", "folding", "visual search")
ggbiplot(pca, choices=2:3, segColor=c("#CC3333", rep("black", 4))) + xlim(c(-2.6,2.6)) +  ylim(c(-2.6,2.6))
ggbiplot(pca, choices=4:5, segColor=c("#CC3333", rep("black", 4)))
@
