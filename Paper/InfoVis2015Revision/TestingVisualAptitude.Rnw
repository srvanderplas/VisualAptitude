...
\hhnote{ these are the references we are supposed to discuss (not sure, how to add the talbot one - that is a study extending cleveland and then heer's). first attempt: It has been shown~\citep{pak2014} that visualizations do not necessarily reflect statistical properties well or are even misleading viewers~\citep{correll2013}. Lineup tests are different as they do not ask participants to judge significances directly but are only based on comparisons. Statistical properties are then derived from this assessment.  This has been shown to be similarly powerful to standard statistical tests in situations where these classical tests hold, and more powerful in situations where conditions of classical tests are not met~\citet{majumder2013validation}. The difference between statistical properties and visualizations is very easy to explain, as statistical significance of, e.g. a correlation coefficient is based on only a few summary statistics, whereas e.g. a corresponding  scatterplot is showing a lot more details of the data which might lead participants to a different evaluation. This, however, is not a drawback of the visualization but only shows that summary statistics might not be enough to come to an appropriate conclusion \citet{anscombe:1973}. }
