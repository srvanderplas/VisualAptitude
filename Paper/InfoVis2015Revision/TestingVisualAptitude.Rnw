...
\hhnote{XXX Still missing: we need to extend the discussion a little bit that we want to extract the common core from lineups rather than investigate the relationship between individual designs and the  tasks. I believe, that the lineup commonalities are stronger than the design differences - but we might need to get some statistical backup for that. ... didn't we run those PCAs as well? }
...
\comment{Not sure where to add the Related Work section; the introduction is really the most similar thing, but adding another sub-section in would really kill the flow of the paragraph, in my opinion.}
\hhnote{I agree, it'll disrupt the flow, but we'll have to add a related work section nevertheless. We could add a bit of discussion on separating lineups from plots judging significance.}


``Lineups" were introduced as a tool to evaluate the statistical significance of a graphical finding~\cite{wickham2010graphical}. Lineups are also useful in assessing the effectiveness of different graphical displays~\cite{hofmann2012graphical, loy:2015}. Like their police counterpart, lineups consist of several distractor plots (of randomly generated data) and one target (the data plot). 
% 
% \begin{figure}[htp]
% 
% \end{figure}

Figure~\ref{fig:lineup}(a) shows a sample lineup of boxplots; participants are expected to identify the most different among the plots shown. In this example, sub-plot 4 is the target  because of the noticeably different locations of the two boxplot medians.
...
\newtext{
Our goal in this study is not to evaluate the statistical significance of particular plots, but rather, to consider the cognitive demands that we require of participants in lineup studies. 
}
...
\hhnote{First attempt: It has been shown~\cite{pak2014} that visualizations do not necessarily reflect statistical properties well or are even misleading viewers~\cite{correll2013}. Lineup tests are different as they do not ask participants to judge significances directly but are only based on comparisons. Statistical properties are then derived from this assessment.  This has been shown to be similarly powerful to standard statistical tests in situations where these classical tests hold, and more powerful in situations where conditions of classical tests are not met~\cite{majumder2013validation}. The difference between statistical properties and visualizations is very easy to explain, as statistical significance of, e.g. a correlation coefficient is based on only a few summary statistics, whereas e.g. a corresponding  scatterplot is showing a lot more details of the data which might lead participants to a different evaluation. This, however, is not a drawback of the visualization but only shows that summary statistics might not be enough to come to an appropriate conclusion \cite{anscombe:1973}. }
\hhnote{XXX still missing: talbot reference; not sure, how to add the talbot one - it is a really nice study  extending cleveland's and then heer's, but not directly related.}
...
\newtext{This phrasing is intentionally ambiguous to prevent biasing participants and is commonly used in other studies employing lineups~\cite{wickham2010graphical, majumder2013validation}.}
...
\newtext{In this experiment, we are interested in individual skill at evaluating lineups rather than the data displayed in a single lineup, so we will consider an individual's aggregate lineup score. 
}
...
\newtext{This study uses the lineup protocol to examine differences in individual performance, rather than using aggregate scores to determine the effectiveness of a single visualization. }

\newtext{Statistical lineup experiments may include the same set of data and distractor plots, visualized using different plot types. This allows researchers to examine whether, for instance, a box plot is more effective than a dot plot at presenting a certain distributional characteristic. This study uses three blocks of statistical lineups; each block consists of 5-7 datasets, each displayed using three or four types of plots. Each block of lineups are a subset of lineups designed to compare the visual significance of different plot types (examples of the visualization types presented in each block of lineups are shown in figure \ref{fig:lineups}). Our purpose in this study is to compare the cognitive demands required to successfully evaluate these datasets, which contain variance difference, mean difference, or violations of normality, as displayed in various common statistical visualizations.}
...
\newtext{All tasks (cognitive and lineups) were provided to participants on paper, to conform to the procedure used in~\cite{ekstrom1976manual}. The order of tasks was the same for each participant due to the constraints of in-person testing; pilot tests did not indicate an effect of task order. }
...
\newtext{After each test or block of lineups, participants were given 1-2 minutes to rest and recover; the order of tests was designed to prevent fatigue on a single task. The duration of testing for each participant was approximately 1 hour. }
...
\end{eqnarray}
\newtext{Equation \ref{eq.scoring} is a restatement of equation \ref{eq.scoring1} provided for convenience.}
...
ggplot() + 
  ...
  #coord_flip()
if (file.exists("./Figure/fig-ResultsScaledScores-1.pdf")) 
  embedFonts("./Figure/fig-ResultsScaledScores-1.pdf")
@
...
<<VisReasoningCategorical, echo=F, include=F, eval=T, fig.width=8.75,fig.height=6, message=F, warning=F, fig.pos='ht'>>=
loadfonts()
...
qplot(data=lineup.summary.categorical, x=value, y=lineup, geom="blank") +
  geom_boxplot(fill="grey", outlier.size=1.5) + 
  facet_wrap(~variable, scales="free_x", ncol=4) + xlab("") + 
  ylab("Scaled Lineup Score") + 
  geom_point(aes(x=value, y=lineup), shape=1, size=2.5) + 
  theme_bw() + 
  theme(plot.margin=unit(c(0,0,0,0), unit="mm"))
if (file.exists("./Figure/fig-VisReasoningCategorical-1.pdf"))
  embedFonts("./Figure/fig-VisReasoningCategorical-1.pdf")
@
...
if (file.exists("./Figure/fig-VisReasoningSPM-1.pdf")) embedFonts("./Figure/fig-VisReasoningSPM-1.pdf")
@
\begin{figure}[ht]
\centering
...
\newtext{While test scores are correlated, the relationships between test scores appear to be linear, with sufficient variability around the trend line to merit further investigation. The correlations are to be expected, as all of the tests in this experiment are conducted in a visual domain, and require general reasoning, intelligence, and other factors. }
What is of more interest to us is 
\newtext{whether lineups recruit specific cognitive abilities, such as visual search, reasoning and categorization ability, mental rotation.} 
...
print(xtable(pca2$rotation, caption="Rotation matrix for principal component analysis of the four cognitive tests (visual search, paper folding, card rotation, figure classification).\\label{tab:PCArotation4}",digits=2), sanitize.text.function=sanitize, sanitize.rownames.function=sanitize, sanitize.colnames.function=sanitize, table.placement="htb", caption.placement="top")
@

<<biplot-pca4, fig.cap="", fig.width=5.5, fig.height=5.5, warning=F, message=F, out.width='.45\\linewidth', comment="|", fig.cap="Biplots of principal components 1-4 with observations (same as figure 1(b)). Principal component analysis was performed on the four cognitive tests used to understand the association between the cognitive skills required for these tests and the skills required for the lineup protocol.  \\label{fig:biplots4}", echo=F, fig.env="figure">>=
#par(mar=c(4,4,.1,.1),cex.lab=.95,cex.axis=.9,mgp=c(2,.7,0),tcl=-.3)
dimnames(pca2$rotation)[[1]] <- c("card\nrotation", "figure\nclassification", "paper\nfolding", "visual\nsearch")

ggbiplot(pca2, choices=1:2) + xlim(c(-2.5,4.1)) +  ylim(c(-2.5,2.5))
ggbiplot(pca2, choices=3:4) + xlim(c(-3,3.5)) + ylim(c(-3,3))
@
...
<<biplots-pca5, fig.cap="", fig.width=5.5, fig.height=5.5, warning=F, message=F, out.width='.45\\linewidth', comment="|", fig.cap="Biplots of principal components 2-5 with observations. The lineup task appears to be most similar to the figure classification task, based on the plot of PC2 vs. PC3. (These are the same biplots as shown in figure \\ref{fig:lineup}b)  \\label{fig:biplots5}", echo=F, fig.env="figure">>=


dimnames(pca$rotation)[[1]] <- c("lineup", "card\nrotation", "figure\nclassification", "paper\nfolding", "visual\nsearch")
ggbiplot(pca, choices=2:3, segColor=c("#CC3333", rep("black", 4))) + xlim(c(-2.4,2.8)) +  ylim(c(-2.6,2.6))
ggbiplot(pca, choices=4:5, segColor=c("#CC3333", rep("black", 4))) + xlim(c(-2.8,2.8)) + ylim(c(-2.8,2.8))
@
...
<<LineupTypeSPM,echo=FALSE,include=F, eval=T, fig.width=6,fig.height=9.75, message=F, warning=F>>=
...
if (file.exists("./Figure/fig-lineupblockpca-1.pdf")) embedFonts("./Figure/fig-lineupblockpca-1.pdf")
@
