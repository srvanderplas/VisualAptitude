...
\hhnote{this next paragraph is a bit confusing ... it seems to suggest that we are using lineups to distinguish between different designs after all. Do we need this paragraph at all? I think it would flow directly from the sentence before to the paragraph after this one.}
...
\hhnote{XXX this procedural description needs a few more pieces: order of different tests (pilot study showed that order did not matter), 
total experimental time, experimental setup: in a lab with paper copies;  }
...
Each of the three sets of 20 lineups was taken from previous experiments examining which type of plot most effectively conveyed important characteristics of the underlying data set. \hhnote{citations?}
