...
\newtext{
The cognitive tests from \cite{ekstrom1976manual} are highly validated tests that have been constructed to measure specific cognitive factors. We have additionally added the VST in order to additionally account for the basic visual search component of the lineup task.
}
...
<<VisReasoningCategorical, echo=F, include=F, eval=T, fig.width=6,fig.height=5, message=F, warning=F, fig.pos='ht'>>=
loadfonts()
...
lineup.summary.categorical$variable <- gsub("Math/Science Research", "Math/Sci Research", lineup.summary.categorical$variable)
...
row.names(pca2$rotation) <- c("Card Rot", "Fig Class", "Folding", "VST")
