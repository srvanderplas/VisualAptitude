...
\newtext{Statistical lineup experiments may include the same set of data and distractor plots, visualized using different plot types. This allows researchers to examine whether, for instance, a box plot is more effective than a dot plot at presenting a certain distributional characteristic. This study uses three blocks of statistical lineups; each block consists of 5-7 datasets, each displayed using three or four types of plots. Each block of lineups are a subset of lineups designed to compare the visual significance of different plot types (examples of each plot type are shown in figure \ref{fig:lineups}). Our purpose in this study is to compare the cognitive demands required to successfully evaluate these datasets, which contain variance difference, mean difference, or violations of normality, as displayed in various common statistical visualizations.}
