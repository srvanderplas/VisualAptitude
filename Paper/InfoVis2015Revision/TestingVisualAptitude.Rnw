...
sanitize <- function(x){
  x %>% 
    ...
    gsub(pattern="Pr\\((.*)\\)", replacement="Pr$(\\1)$")
}
...
All tasks (cognitive and lineups) were provided to participants on paper in a supervised lab setting, to conform to the procedure used in~\cite{ekstrom1976manual}. The order of tasks was the same for each participant due to the constraints of in-person testing; an internal pilot study did not indicate an effect of task order, nor did the pilot results suggest an interaction/learning effect from one task to the next.
...
After each test or block of lineups, participants were given 1-2 minutes to rest and recover; the order of tests was designed to prevent fatigue on a single task (which led us to interweave lineup evaluations). The duration of testing for each participant was approximately 1 hour. None of the participants who showed up for their scheduled session asked to leave early. A complete set of test documents (with copyrighted material redacted) is available as supplemental materials. 
...
# if (file.exists("./Figure/fig-VisReasoningSPM-1.pdf")) 
...
ggplot()+
  ...
  theme_bw() + 
  theme(axis.title=element_blank())
# if (file.exists("./Figure/fig-LineupTypeSPM-1.pdf")) 
...
# rotation$Var3 <- str_replace(rotation$Var1, pattern="lineup_\\d", replacement="Lineup-Core")
# rotation$Var3 <- factor(rotation$Var3, 
...
# if (file.exists("./Figure/fig-lineupblockpca-1.pdf")) 
