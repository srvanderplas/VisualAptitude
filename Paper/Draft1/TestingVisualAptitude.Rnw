...
\newtext{PC2 and PC3 sseparate the paper folding, visual search, and figure classification tasks, while PC4 and PC5 separate lineups, card rotation, figure classification, and visual search. This separation allows us to compare the tasks which are similar from among the principal components. }
