...
ggplot() + 
  ...
  theme(legend.position=c(.85,.825),  legend.box.just = "left", 
...
Results from the visuospatial tests used in this experiment are highly correlated, as shown in figure~\ref{fig:scatterplotmatrix}; this is to be expected given that all of these tests are measuring visual ability of individuals in some way. 
%are influenced by factors other than visual ability -- general intelligence, mental processing speed, cognitive resources, motivation, and attention, to name a few. 
...
In order to assess factors contributing to lineup performance, we first examine the separate dimensions measured by the entire battery of tests using principal components analysis on the scaled test scores. 
\comment{give a mini-paragraph summary of the PCA of four}
%
\newtext{A principal component analysis (PCA) of the four established visuo-spatial tests reveals that they all share a very strong first component, which explains about XXX\% of the total variability. PC1 is essentially an average across all tests representing a general ``visual intelligence" factor. The principal components beyond that span another two dimensions, while the last dimension is weak (at XXX\%). The second principal component differentiates the figure classification task from card rotation, PC3  ... PC4 ...}

\comment{then move on to the PCA of five, but summarize strongly - what does this one have in common with the PCA of four. The comments from below would be great here.}
Table~\ref{tab:PCAvariance5} shows the importance of each principal component and the cumulative proportion of variance. 
PC1
accounts for \Sexpr{round(summary(pca)$importance[3,1]*100,1)}\% of the variance in the data, the corresponding rotation matrix is provided in~\ref{tab:PCArotation5}.
As expected, PC1 is essentially an average across all tests representing a general ``visual intelligence" factor. %, that is, all of the general factors discussed above which influence test results. 
...
Biplots of these principal components are shown in figure~\ref{fig:biplots5}. 
...
\comment{The following sentences are great, but would need to move up a bit, I think}
...
\comment{that last sentence is probably reaching a bit far... \textit I think that sentence is just fine :)}
...
\begin{eqnarray}
\mu_{\text{all}} &= (\mu_{F}N_F + \mu_{M}N_M)/(N_F + N_M)\\
\sigma_{\text{all}} &= \sqrt{\left(N_F\sigma_F^2 + N_M\sigma_M^2\right)/(N_F + N_M)},
\end{eqnarray}
\newtext{where $\mu_F$ and $\mu_M$ are the averages observed individually, ...}
\comment{You need to define all elements in the calculations}

\comment{In a next step, include the derivations using the actual numbers.}

\comment{We can approach the next piece a bit more generally.}
\newtext{Whenever participants in two studies were not exposed to the same number of questions, the resulting scores are not comparable: both overall scores and their standard deviations are different. We can achieve comparability by scaling the scores accordingly.}

In order to account for the fact that ISU students took only part I of two parts to the figure classification test (and thus completed half of the questions), we adjust the transformation as follows:
...
Table~\ref{tab:ttest-demographics} provides the results of a sequence of linear models fit to the lineup data. Each row in the table represents a single model, with one predictor variable (a factor with two or more levels).
\comment{Comment on a few more things here: order of variables in the table, and what exactly you have done to compute the values (model), also, why did you use separate factors for the skills and the number of video game hours? All of these variables are ordinal, so a linear factor using a single degree of freedom might be the most suitable for a start.}
%Due to sample size considerations, multiple testing corrections were not performed;
Note that these covariates are correlated: in our sample, males are more likely to have completed Calculus 1, but are also more likely to spend time playing video games. 
\comment{the next sentence will need some re-phrasing according to the results of the linear model.}
As such, a model including two or more of the significant predictor variables shows all included variables to be nonsignificant. To better understand the effects of these variables, a much larger study would be required. 
...
\section{Principal Component Analysis of Visuo-Spatial Tests}\label{app:pca4}
