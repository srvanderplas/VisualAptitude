...
@
Figure~\ref{fig:biplots4} shows that the first PC does not differentiate between any of the tasks; it might be best understood as a general aptitude factor. All of the following principal components separate between the cognitive tasks.
\comment{How do the PCs exactly separate? Quick sentence for each.}
