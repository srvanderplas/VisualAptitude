...
\newtext{Applying the scaling protocol discussed in section \ref{sec:scaling}, we see that the ranges of scaled lineup and visuospatial test scores do not include zero; this indicates that we do not see random guessing from participants in any task.} 
