...
\textbf{Done; added to Results section}\\
...
\textbf{Done, along with an examination of each experiment separately}\\
...
\begin{figure}[htp]
<<VisReasoningSPM,echo=FALSE, include=T, eval=T, fig.width=8,fig.height=8, message=F, warning=F>>=
