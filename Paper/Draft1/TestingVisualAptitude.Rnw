...
\comment{Susan, there's only a few things left to do, mostly to do with explanations for figures. x-axis, y-axis, ... Have a look at fig 6 again. It is a great chart, but it needs some explanation because it is a non-standard chart that you thought of.

Fig 22 and 23 also need a bit more explanation.}
...
n_c - n_w/(m-1).
\end{eqnarray}
Note that the sum of answers, $n_c + n_w$, is at most $K$, but could be less, if an observer chooses to not answer one of the lineup tasks or runs out of time.
...
Biplots \cite{biplots:gabriel} of the remaining components are provided in Appendix~\ref{app:pca5}. 
...
% 
% \comment{Susan, sorry, the description of figure 6 is still not working. You write impact at the x axis - so I'm wondering what a negative impact is supposed to be? You are mentioning contribution and importance in the paragraph below, but you don't really define either one of them.
% Be careful about mentioning Table 2 - that table doesn't have anything to do with this PCA - it might be better to include the two tables for this PCA in the appendix and work out there in detail, how the scaling is done. InfoVis does know about signular value composition, so you could start there ... }

\comment{PC$_j$ needs to be defined - I believe that you started out with the idea of calling it $Y_j$. The woulds and coulds need to go. Use the word 'define' so that it is absolutely clear that  it's not something you hypothetically 'could calculate'.}
\newtext{
Principal component analysis separates multivariate data into orthogonal components using a rotation matrix to transform correlated input data into an orthogonal space. The importance of a principal component is also evaluated to assess the proportion of the overall variance in the data contained within the component (Table \ref{tab:PCAvariance5} shows the importance of each PC for the analysis of the aggregate lineup score and the four aptitude tests). For input variables $X_i$, $i=1, ..., K$ and output variables $Y_j$, $j=1, ..., K$, the rotation matrix, $M$ would have dimension $K\times K$ (indexed by $i$ and $j$ respectively), and the importance vector $I$ would have dimension $1\times K$ (indexed by $j$). For each $X_i$ and $Y_j$, we could calculate the influence of variable $X_i$ on component $Y_j$ as follows:

$$\text{Influence of variable} X_i \text{ on PC}_j = M_{ij}\times I_j$$

The influence is a measure of the contribution of an input variable to the principal component, scaled by the importance of that principal component to the overall variability in the data. 

Figure~\ref{fig:lineupblockpca} shows the influence of each input variable on each principal component for a principal component analysis  including each lineup task as a separate input variable. The input variables are shown on the $y$ axis, with the influence of the variable on each PC shown on the $x$ axis. This allows us to consider the rotation matrix visually (while accounting for the importance of each principal component); for instance, we see that again, the first PC accounts for most of the variance and seems to represent general visual aptitude. 
}
<<lineupblockpca, echo=F, warning=F, message=F, fig.cap="Principal Component influence plot, showing each input variable's contribution to the principal component, scaled by the proportion of variability in the data contained in the principal component.", fig.width=5, fig.height=5>>=
...
qplot(x=0,  y=Var1, xend=value*importance, yend=Var1, geom="segment", data=rotation, arrow=arrow(length=unit(0.1, "cm"))) + facet_wrap(~Var2, ncol=3) + xlab("Influence") + ggtitle("Lineup Types and Visual Aptitude") + ylab("") + theme_bw()
@


% \comment{add something about x and y axis in the description of the figure. }
% \comment{Let's give this plots a name ... how about {\it PC importance plots}? Get rid of the * in the x axis title. Both `realtive contribution' and `importance' are a bit fuzzy terms ... I'm not sure what you are doing until I'm looking at the code. Define importance and each of the items, and yes, that might need a bit of math.}
...
% \comment{reference is gone astray `those who score' ... the previous subject were facets of intelligence, and they do not score on tests - what you mean are people, so say that!}
...
% \comment{'Results' of lineup score - the caption in the table is too generic. give more details. }
...
<<biplots-pca5, fig.cap="", fig.width=4.5, fig.height=4.5, warning=F, message=F, out.width='.7\\linewidth', comment="|", fig.cap="Biplots of principal components 2-5 with observations. The lineup task appears to be most similar to the figure classification task, based on the plot of PC2 vs. PC3.  \\label{fig:biplots5}", echo=F, fig.env="figure">>=
par(mar=c(4,4,.1,.1),cex.lab=.95,cex.axis=.9,mgp=c(2,.7,0),tcl=-.3)
...
\comment{For all of these figures: include first, what you are showing: x axis, y axis, ... before you go into an interpretation of the figure.}
<<lineup-types,echo=F, fig.cap="Density plots of scaled scores for different types of lineups. For the same experiment (shown by line color), certain types of plots are more difficult to read and are associated with lower participant scores. \\label{fig:plottypesdensity}", fig.width=4, fig.height=8, out.width=".9\\linewidth", warning=F, message=F>>=
...
<<lineup-types-scores,echo=F, fig.cap="Scatterplots of scaled lineup scores by aptitude test scores. There is some indication that different types of lineup tasks may utilize different visual skills; for instance, QQ plots with confidence bands may require more skill at mental rotation than QQ plots without the bands. \\label{fig:scatterplottypes}", fig.width=6, fig.height=12, out.width="\\linewidth", fig.env="figure", warning=F, message=F>>=
