...
qplot(data=lineup.summary.categorical, x=value, y=lineup, geom="boxplot") + facet_wrap(~variable, scales="free_x") + xlab("Value") + ylab("Lineup Score (# Correct - 1/19 # Incorrect)") + theme_bw() + geom_point(aes(x=value, y=lineup), shape=1, size=3)
