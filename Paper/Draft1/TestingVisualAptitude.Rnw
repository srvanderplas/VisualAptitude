...
\comment{More lineup introduction here}

Statistical lineups~\cite{hofmann2012graphical,buja2009statistical} depend on the ability to search for a signal amid distractors (visual search) and the ability to infer patterns from stimuli (pattern recognition). Some lineups (polar coords) also depend on the ability to mentally rotate stimuli (spatial rotation) and mentally manipulate graphs (spatial rotation and manipulation). By breaking the lineup task down into component parts, we may be able to determine which visuospatial factors most strongly correlate with lineup performance, using carefully chosen cognitive tests to assess these aspects of visuospatial ability. \newtext{Demographic factors are known to contribute to lineup performance: country, education, and age affected score on lineup tests, and all of those factors plus gender affected the amount of time spent on lineups \cite{humanfactorslineups}.}\newtext{In addition, lineup performance can be partially explained using statistical distance metrics \cite{distancemetriclineups}, but these metrics do not completely succeed in predicting human performance, in part due to the difficulty of representing human visual ability algorithmically.}\newtext{In this paper, we examine some demographic characteristics as well as characteristics such as  spatial and pattern recognition ability, with the goal of understanding the skills necessary for accurate lineup performance as well as underlying variation in lineup performance. }

\section{Methods}
\subsection{Measures of visuospatial ability}

Participants will complete several cognitive tests designed to measure spatial and reasoning ability. Tasks are designed so that participants are under time pressure; they are not expected to complete all of the problems in each section. This provides more discrimination between high scorers and prevents score compression at the top of the range. 

The \textbf{visual searching task} (VST), shown in figure~\ref{fig:VST}, is designed to test participants' ability to find a target stimulus in a field of distractors. The visual search task is similar in concept to lineups: it tests an individual's ability to find the target plot. Historically, it has been used as a measure of brain damage\cite{goldstein1973validity,demita1981validity,moerland1986neuropsychological}; however, similar tasks, have been used to measure cognitive performance in a variety of situations (under the influence of drugs, for example, in~\cite{anderson1983interactive}). The similarity to the lineup protocol as well as the simplicity of the test and its' lack of color justify the slight deviation from forms of visual search tasks typically used in normal populations. 
...
\newtext{The card rotation test is often used in studies of the effect of visual ability on the use of visual aids \cite{mayer1994whom} and statistical graphs \cite{lowrie2007solving} in education.}
\begin{figure*}[hbt]\centering
...
In addition to these tests, participants were asked to complete a questionnaire which includes questions about colorblindness, mathematical background, self-perceived verbal/mathematical/artistic skills, time spent playing video games, and undergraduate major. \comment{XXX should we classify majors into STEM/non-STEM? - there's a ton of literature and interest that we could potentially tap into \\\textit{I'm not opposed, but we have to decide where ag-majors fall... not everything is easily classifiable. Psychology is another problem: I was very STEM-psychology, but you don't have to be.}} These questions are designed to assess different factors which may influence a participant's skill at reading graphs and performing spatial tasks. 
...
% \comment{maybe reference book by Michel Hersen\\\textit{I was working on it yesterday along with trying to validate the general idea that reading graphics requires certain spatial abilities. It's definitely a work in progress. I haven't seen that book before, but I'm not at all surprised it exists.}}

% 
% \subsection{Variance consideration of scaled test score:}
...
\section{Results}
Results are based on an evaluation of \Sexpr{nrow(ans)} undergraduate students at Iowa State University. About one-half of the participants were in STEM fields, the others were distributed relatively evenly between agriculture, business, and the social sciences. Students were evenly distributed by gender, and were between 18 and 24 years of age with only one exception. This is reasonably representative of the university as a whole; in the fall 2012 semester, 23\% of students were associated with the college of engineering, 23\% were associated with the college of liberal arts and sciences, 23\% were associated with the college of human sciences, 12\% with the business school, and 14\% with the school of agriculture. \footnote{\url{http://www.ir.iastate.edu/PDFfiles/2012-2013\%20Student\%20Profile.pdf}} 
...
ggplot() + 
  ...
  scale_colour_discrete(name="", labels="Theoretical\nRange") +
  scale_fill_discrete(name="", labels="Scores\nindicating\nguessing") +
  theme_bw()  + 
  ylab("Test Scores") + 
  xlab("") +
  theme(axis.text.x = element_text(angle = 30, hjust = 1), legend.position=c(.75,.8)) +
...
<<VisReasoningCategorical, echo=F, include=F, eval=T, fig.width=9,fig.height=5, message=F, warning=F, fig.pos='htb'>>=
...
qplot(data=lineup.summary.categorical, x=value, y=lineup, geom="boxplot") + 
  facet_wrap(~variable, scales="free_x", ncol=5) + xlab("") + 
...
\caption[Visual Aptitude Study Results]{Demographic characteristics of participants compared with lineup score. Categories are ordered by effect size; calculus completion,  hours spent playing video games per week, and sex are all associated with a significant difference in lineup score. }\label{fig:visualaptitudecat}
\end{figure*}

\newtext{Next, we examine the effect of demographic factors on lineup performance. Previous work found a relationship between lineup performance and factors such as education level, country of origin, and age\cite{humanfactorslineups}; our participant population is more homogeneous, which allows us to explore factors such as educational background and skills on performance in lineup tests. }

Figure~\ref{fig:visualaptitudecat} shows participants' lineup scores as a function of their responses to the questionnaire given at the beginning of the study; these demographic questions allow us to explore relationships between demographic characteristics (major, research experience, etc.) and test performance. 

Completion of Calculus I is associated with increased performance on lineups; this may be related to general math education level, or it may be that success in both lineups and calculus requires certain visual skills. This association is consistent with~\cite{shah1995conceptual}, who found an association between  mathematical ability and performance on simple graph description tasks.  There is also a significant association between hours of video games played per week and score on lineups, however, this association is not monotonic and the groups do not have equal sample size, so the conclusion may be suspect. There is a (nearly) significant difference between male and female performance on lineups; this is not particularly surprising, since men perform better on many spatial tests~\cite{voyer1995magnitude} and performance on spatial tests is correlated with phase of the menstrual cycle in women~\cite{hausmann2000sex}. There is no significant difference in lineup performance for participants of different age, self-assessed skills in various domains, previous participation in math or science research, completion of a statistics class, or experience with AutoCAD. These demographic characteristics were chosen to account for life experience and personal skills which may have influenced the results. Statistical test results are available in appendix~\ref{app:categoricalresults}. 
% <<means>>=
% library(plyr)
...
<<pcatests,echo=F, fig.cap="Scree plot of principle component analysis of performance on the different test batteries.", out.width=".8\\linewidth", fig.height=3, fig.width=4.5, warning=F, message=F, size="footnotesize">>=
pca <- prcomp(ans.summary[,c("lineup", "card_rot", "fig_class", "folding", "vis_search")], scale=T, retx=T)
# screeplot(pca)
@

\newtext{Results from the visuospatial tests used in this experiment are highly correlated, as shown in figure~\ref{fig:scatterplotmatrix}; this is to be expected given that all of these tests are influenced by factors other than visual ability - general intelligence, mental processing speed, cognitive resources, motivation, and attention, to name a few. In order to assess factors contributing to lineup performance, we first will examine the separate dimensions measured by the entire battery of tests using principal components analysis on the scaled test scores.} 
\newtext{Table \ref{tab:PCAvariance5} shows the importance of each principal component and the cumulative proportion of variance. As expected, PC1 accounts for \Sexpr{round(summary(pca)$importance[3,1]*100,1)}\% of the variance in the data, that is, all of the general factors discussed above which influence test results. We will focus on the remaining components; these show differences in performance on the tests, and thus provide information about the skills needed to successfully examine lineups. }

\newtext{Biplots of the remaining principal components are shown in figure~\ref{fig:biplots5}, and the corresponding rotation matrix is provided in \ref{tab:PCArotation5}. PC1 seems to represent a general ``visual intelligence'' factor.}

<<pcasummary,echo=F, results='asis'>>=
...
@
<<pcasummary2,echo=F, results='asis'>>=
...
@
\newtext{If we examine the relationship between the four cognitive tests (excluding lineups), we can determine how lineup performance affects the variance partitions. Table \ref{tab:PCAvariance4} contains the importance of each resultant PC and the proportion of the variance each PC represents. }

\newtext{As with the previous principal components analysis, PC1 accounts for about \Sexpr{round(summary(pca2)$importance[3,1],1)*100}\% of the variance; figure \ref{fig:biplots4} and table \ref{tab:PCArotation4} confirm that PC1 is again a measure of the similarity between all 4 tests; that is, a participant's general (or visual) aptitude. PC2 differentiates the figure classification test from the visual searching test; this is incredibly similar to the role of PC2 in the 5-component analysis. }

<<pcarotation2,echo=F, results='asis'>>=
...
\newtext{PC3 differentiates the figure classification and visual search tests from the paper folding test; this is true in the 5-component analysis as well, but in that analysis the role of the figure classification test was not as significant due to the inclusion of the lineup task. PC4 is not particularly significant (it accounts for \Sexpr{round(summary(pca2)$importance[2,4]*100,1)}\% of the variance), but it differentiates the card rotation task from the paper folding task, much like PC5 in the full PCA.}

\newtext{Comparing the two principal component analyses in this manner suggests that lineups are a separate dimension (as the addition of lineups result in an additional principal component which accounts for at least 10\% of the variance), but that they are most closely related to the figure classification task, and less closely related to the visual searching task.}
...
% 
% \begin{figure}[hbt]\centering
% \includegraphics[width=0.75\linewidth]{fig-VisReasoningCts-1}
...
Unscaled Scores & 44.0 (24.6)\footnote{Data from Part I only.}
                ...
                & M:~120.0~(30.0), F:~114.9~(27.8)\\
{\footnotesize Population}    
              & \parbox[t]{.2\linewidth}{\footnotesize approx.\ 550 male \\
naval recruits} 
              & \parbox[t]{.2\linewidth}{\footnotesize 46 college students\\(1963~version)}
              & \parbox[t]{.3\linewidth}{\footnotesize suburban 11th \& 12th grade students\\(288-300 males, 317-329 females)}\\\hline
\end{tabular}                
\end{table*}
\end{savenotes}
...
tmp <- rbind.fill(tmp)
tmp <- tmp[order(tmp$p.value, decreasing=F),]
print(xtable(tmp, digits=c(1, 1, 0, 3, 3, 2, 3), caption="Model results of each single demographic variable compared with lineup score. Multiple testing issues aside, it appears that very few demographic variables (if any) are significantly associated with score on lineups among Iowa State undergraduate students."), include.rownames=F, floating.environment="table*", caption.placement="top")
@
...
<<partialregression, results='asis', echo=1, tidy=T>>=
...
@
<<partialregression1,fig.keep='all', fig.width=4, fig.height=4, out.width='.45\\linewidth', fig.cap="Biplots of the adjusted lineup, card rotation, figure~classification, and paper folding scores principal components analysis. Adjusted lineup scores are most associated with figure~classification in the second principal component. In the third and fourth components, adjusted scores are almost entirely orthogonal. \\label{fig:biplot-adjusted}", tidy=T, echo=1:7>>=
...
print(xtable(model2, caption="Results of the linear regression of (the ETS tests $|$ VST) on (lineup score $|$ VST)"), sanitize.text.function=sanitize, caption.placement="top")
@
...
print(xtable(model3, caption="Results of the reduced linear regression of (the ETS tests $|$ VST) on (lineup score $|$ VST)"), sanitize.text.function=sanitize, caption.placement="top")
print(xtable(anova(model3, model2), caption="Model comparison between full model (including paper folding) and reduced model (excluding paper folding), when accounting for scores on the VST in both independent and dependent variables. "), sanitize.text.function=sanitize, caption.placement="top")
@
...
They also cite a bunch of studies (including authors in~\cite{voyer1995magnitude}) that I haven't had time to completely explore.}}
