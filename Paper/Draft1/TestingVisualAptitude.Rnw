...
\begin{abstract}
\comment{Tone down the `statistical' part ... statistical, to the wrong ears sounds complicated. } Graphics convey numerical information very efficiently, but rely on a different set of mental processes than tabular displays. 
\newtext{}
...
\IEEEPARstart{D}{ata} displays provide quick summaries of data, models, and results, but not all displays are equally good, nor is any data display equally useful to all viewers. 
...
% \comment{I am not sure what the kosara reference has to do with the lineups - the only thing lineups and kosara have in common are the amazon studies. but you might want to keep the reference around for a discussion of the amazon results }
``Lineups" have recently been introduced \cite{buja2009statistical, wickham2010graphical, majumder2013validation} as a tool to evaluate the statistical significance of a graphical finding. Lineups are also useful in assessing the effectiveness of different graphical displays \cite{hofmann2012graphical, loy:2015}. Like their police counterpart, lineups consist of several distractor plots (of randomly generated data) and one target (the data plot). 
...
% 
% \comment{`While previous research' - which research are you referring to? 
% Yifan's paper with the eyetracking study has looked at the spatial connection \cite{zhao2013mind}. But is that what you mean?
% 
% `required by the lineup task' - that makes the lineup task strangely active. Could you rephrase that in terms of the participant?
% }
...
% Exploring the participant characteristics which contribute to successful lineup evaluation allows us to better understand the lineup task and potential sources of variability in the study results. 



\comment{good discussion, but this is the first time that you mention `our experiment' - include a sentence here, to say what this experiment is about. also: I think we should only say `study' or `experiment', and I'd prefer study, because experiment sounds like it could go BOOM. I don't want to give that impression. }

Our experiment compares lineup performance with established tests, such as the visual search task (VST), paper folding test, card rotation test, and figure classification test. 
\comment{include citations for the tests}
The VST measures visual search speed, the paper folding and card rotation tests measures spatial manipulation ability, and the figure classification test measures inductive reasoning; 
...
We hope to facilitate comparison of the lineup task to known cognitive tests, inform the design of future studies, and better understand human of the perception of statistical lineups. 
\comment {something went wrong with the last half sentence ... do you mean the human component in the statistical lineups?}


% 
% \comment{This is going into the right direction, but a few pieces are still missing, 
% you need to still say what this paper is doing: we do a study to compare lineup performance with established tests, also go into a bit of discussion of the tests.}
% \comment{Here is a paragraph missing, that connects the existing literature to what we are doing in this paper. 
% In particular, we need to answer the questions: 
% 
% - why are we interested in evaluating which skills are necessary to evaluate graphical displays (this has been implicitly answered by saying that cognitive load was reduced, so we free up resources and make graphics a better tool for more people.
% 
% - what are the limits of the studies that you have cited, ie is there something that we can do better?
% 
% - what advantage do lineups bring to the table?
% }
% 
% 
% \comment{How do perceptual principles apply to statistical graphics (data displays)? - Lineups help us to isolate different parts of displays and allow in-built quantification of results.}
% 
% 
% \comment{Why are lineups variable? - Individual variability is big, we want to investigate which abilities individuals help the most in evaluating. - for this reason we need to motivate why lineups are useful. }
% 
% 
% \comment{We examine 'some demographic characteristics' is a bit lukewarm. I think once the missing paragraph is there, you can sell the rest of the paper a bit more passionately.}

\comment{and in the last paragraph of the intro: lay out the roadmap for the rest of the paper. In section \ref{sec:methods} we introduce ... }
...
This aspect of lineups provides an important tool in the evaluation of graphical findings. However, the testing situation can be reversed, and lineups allow for an objective evaluation of an individual's visual ability based on the general's population performance on a lineup.

}

\comment{Include how an individual's performance is evaluated. }
...
\comment{Let's put figure \ref{fig:tests} into the appendix - that'll save us quite a bit of space. }
 \begin{figure*}[ht]
  \centering
...
Between cognitive tasks, participants were also asked to complete three blocks of 20 lineups each, assembled from previous studies~\cite{hofmann2012graphical,majumder2013validation}. Participants have 5 minutes to complete each block of 20 lineups. Figure~\ref{fig:lineup} shows a sample lineup of box plots. 
...
\comment{here are the numbers for Fall 2014: http://www.registrar.iastate.edu/enrollment}
...
\begin{figure}[ht]
<<ResultsScaledScores, echo=F, include=T, fig.width=4.5, fig.height=4.5, out.width='\\linewidth', warning=F, message=F>>=
...
Biplots of the remaining components are provided in Appendix \ref{app:pca5}. 
...
<<LineupTypeSPM,echo=FALSE,fig.pos="ht", include=T, eval=T, fig.width=4,fig.height=6.5, message=F, warning=F, fig.cap="Pairwise scatterplots of test scores, with lineups separated into the three lineup tasks. All lineup tasks are moderately correlated with the figure classification task, and while tasks 1 and 2 are most strongly correlated with figure classification, lineup task 3 is most strongly correlated with the card rotation and paper folding tasks. \\label{fig:lineuptaskmatrix}">>=
...
<<lineupblockpca, echo=F, warning=F, message=F, fig.cap="Lineup block PCA rotation matrix in graphical form.", fig.width=5, fig.height=5>>=
...
qplot(x=0,  y=Var1, xend=value*importance, yend=Var1, geom="segment", data=rotation, arrow=arrow(length=unit(0.1, "cm"))) + facet_wrap(~Var2, ncol=3) + xlab("Contribution to each PC * % Variance in PC") + ggtitle("Lineup Types and Visual Aptitude") + ylab("") + theme_bw()
@

\comment{First: what is shown in figure 8? THis is a non traditional plot, so you need to explain it first.}
...
%\paragraph*{Removing the effect of overall aptitude}
...
\comment{These read like the conclusions from earlier - video games aren't that prominently displayed anymore unless you are referring to figure 5. But if so, I would be cautious about summarising STEM major as 'mathematical training' - use the same names for the variables as in figure 5 and make sure to keep the order, i.e. gender is more impotant than video game hours. also make sure, that to mention that this is on a univariate level and doesn't take the correlation between the variables into account. }

Performance on lineups is strongly related to performance on tests of visual ability; however, this relationship is mediated by factors such as mathematical training, hours of video games played per week, and gender.
In addition, many facets of intelligence are highly correlated; those who score higher on general aptitude tests may score higher on tests of visual ability (and may also score higher on lineup tests).
...
While it would be impractical to require participants to score well on several different tests, it would be reasonable to ask participants to pre-qualify for a task by completing a figure classification test.
As the figure classification test is different from the lineup task, this would not bias participants' scores on the domain of interest, but would ensure a participant pool that is sufficiently motivated to complete the lineup questions.
...
\end{align}
where $\mu_F$ and $\mu_M$ are the mean of female and male scores respectively, $N_F$ and $N_M$ are the number of participants in each group, and $\sigma_F^2$ and $\sigma_M^2$ are the variance of each group.
...
\comment{'Results' of lineup score - te caption in the table is too generic. give more details. }
...
\comment{'Importance of principal components {\it in an analysis} of four tests' -- be more precise: you mean the PCA. Give some details as to how importance is measured. }
%
...
PC1 accounts for about \Sexpr{round(summary(pca2)$importance[3,1],1)*100}\% of the variance; figure~\ref{fig:biplots4} and table~\ref{tab:PCArotation4} confirm that PC1 is a measure of the similarity between all 4 tests; that is, a participant's general (or visual) aptitude. PC2 differentiates the figure classification test from the visual searching test, while PC3 differentiates these two from the paper folding test. PC4 is not particularly significant (it accounts for \Sexpr{round(summary(pca2)$importance[2,4]*100,1)}\% of the variance), but it differentiates the card rotation task from the paper folding task.
...
@
\comment{figure 9 and 10 are Biplots, say so in the caption - in the discussion of the figures and the tables in the text, please think again of adding the sentence summarising what is shown. }
...
<<biplots-pca5, fig.cap="", fig.width=4.5, fig.height=4.5, warning=F, message=F, out.width='.7\\linewidth', comment="|", fig.cap="Plots of principal components 2-5 with observations. \\label{fig:biplots5}", echo=F, fig.env="figure">>=
par(mar=c(4,4,.1,.1),cex.lab=.95,cex.axis=.9,mgp=c(2,.7,0),tcl=-.3)
...
\comment{Describe performance - need to get data from original experiment.}
\begin{figure}[htbp]
...
<<lineup-types,echo=F, fig.cap="Plot of scaled scores for different types of lineups\\label{fig:plottypesdensity}", fig.width=4, fig.height=8, out.width=".9\\linewidth", warning=F, message=F>>=
...
<<lineup-types-scores,echo=F, fig.cap="Plot of scaled lineup scores by aptitude test scores.\\label{fig:scatterplottypes}", fig.width=6, fig.height=12, out.width="\\linewidth", fig.env="figure", warning=F, message=F>>=
