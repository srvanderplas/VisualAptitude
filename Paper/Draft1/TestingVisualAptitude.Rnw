...
\comment{You'll have to define the use of PCX still - a quick sentence on the order of the principal components being given by the amount of variance in the data that they explain should do it.}
...
\newtext{Figure~\ref{fig:lineupblockpca} shows the relative contribution of each input variable  to each principal component (PC), weighted by the importance of each principal component. The input variables are shown on the $y$ axis, with the importance to each PC shown on the $x$ axis. This allows us to consider the rotation matrix visually; for instance, we see that again, the first PC accounts for most of the variance and seems to represent general visual aptitude. }
