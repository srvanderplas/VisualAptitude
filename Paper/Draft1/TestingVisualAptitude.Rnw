...
\newtext{In this paper, we present the results of a study designed to compare lineup performance with visual aptitude and reasoning tests, examining the skills necessary to successfully evaluate lineups. }
...
\newtext{Figure \ref{fig:lineupblockpca} shows the relative contribution each input variable makes to each principal component, weighted by the importance of each PC. The input variables are shown on the $y$ axis, with the importance to each PC shown on the $x$ axis. This allows us to consider the rotation matrix visually; for instance, we see that again, PC1 accounts for most of the variance and seems to represent general visual aptitude. }
...
\newtext{Figure \ref{fig:biplots4} shows that the first PC does not differentiate between any of the tasks; it might be best understood as a general aptitude factor. PC2 begins to separate the four cognitive tests; PC3 and PC4 also differentiate the four tests. According to table \ref{tab:PCArotation4}, the first three principal components account for \Sexpr{round(summary(pca2)$importance[3,3]*100,1)}\% of the variance.} 
