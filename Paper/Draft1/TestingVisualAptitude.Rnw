...
\newtext{
Principal component analysis separates multivariate data into orthogonal components using a rotation matrix to transform correlated input data into an orthogonal space. The importance of a principal component is also evaluated to assess the proportion of the overall variance in the data contained within the component (Table \ref{tab:PCAvariance5} shows the importance of each PC for the analysis of the aggregate lineup score and the four aptitude tests). For input variables $X_i$, $i=1, ..., K$ and output components $PC_j$, $j=1, ..., K$, the rotation matrix, $M$ has dimension $K\times K$ (indexed by $i$ and $j$ respectively), and the importance vector $I$ has dimension $1\times K$ (indexed by $j$). For each $X_i$ and $PC_j$, we define the influence of variable $X_i$ on component $PC_j$ as follows:
