...
From figure~\ref{fig:lineupblockpca} we see that again, PC1 accounts for general visual aptitude. PC2, however, emphasizes the relationship between figure classification and Lineup task 1 (and to a lesser extent, on lineup task 2). PC3 differentiates the lineup task 3 from the visual search task; other tasks do not impact PC3 to a meaningful degree. PC4 separates lineup test 2 from the paper folding and card rotation tests and emphasizes the overlap with the visual search test. PC5 separates lineup test 2 and the figure classification test from the overlap between the other lineup tests and the visual search test. PC6 and PC7 account for \newtext{joint total of less than 10\% of the overall variability, and do not display any distinct patterns in the loadings.} %\Sexpr{as.numeric(round(100*importance[6], 1))}\% and \Sexpr{as.numeric(round(100*importance[7], 1))}\% of the variance, respectively; PC6 seems to differentiate the first two lineup tasks from the overlap between the third lineup task and the figure classification task.

\comment{What is the overall conclusion?}

\subsection{Lineup Plot Types}
\comment{Susan, would you agree in the assessment that the results from this section are inconclusive at best? Unless I am overlooking anything, I don't see substantial findings. We could move this section to the appendix, and just refer to it as being inconclusive.}
\newtext{
