...
\newtext{
Principal component analysis separates multivariate data into orthogonal components using a rotation matrix, which contains information on the contribution of each input variable to the principal component. The importance of a principal component is also evaluated to assess the proportion of the overall variance in the data contained within the component (Table \ref{tab:PCAvariance5} shows the importance of each PC for the analysis of the aggregate lineup score and the four aptitude tests). 
}
<<lineupblockpca, echo=F, warning=F, message=F, fig.cap="Principal Component importance plot, showing each input variable's contribution to the principal component, scaled by the proportion of variability in the data contained in the principal component.", fig.width=5, fig.height=5>>=
...
Figure~\ref{fig:lineupblockpca} shows the influence of each input variable on each principal component; the arrows are scaled in proportion to the importance of each principal component. The input variables are shown on the $y$ axis, with the influence of the variable on each PC shown on the $x$ axis. This allows us to consider the rotation matrix visually; for instance, we see that again, the first PC accounts for most of the variance and seems to represent general visual aptitude. 
...
<<lineup-types,echo=F, fig.cap="Density plots of scaled scores for different types of lineups. XXX Main message?\\label{fig:plottypesdensity}", fig.width=4, fig.height=8, out.width=".9\\linewidth", warning=F, message=F>>=
...
<<lineup-types-scores,echo=F, fig.cap="Scatterplots of scaled lineup scores by aptitude test scores. XXX main message?\\label{fig:scatterplottypes}", fig.width=6, fig.height=12, out.width="\\linewidth", fig.env="figure", warning=F, message=F>>=
