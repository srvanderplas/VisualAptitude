...
The \textbf{figure classification task} tests participant's ability to extrapolate rules from provided figures. This task is associated with inductive reasoning ablities (factor I in \citet{ekstrom1976manual}). An example is shown in figure \ref{fig:figureclassification}. 
\begin{figure}[htbp]\centering
