...
\section{Results}
\newtext{Results are based on evaluation of \Sexpr{nrow(lineup.summary)} undergraduate students at Iowa State University. }
%We have tested students, and plan to increase sample size once the summer semester begins. 
Preliminary results are presented graphically in figures \ref{fig:visualaptitudects} and \ref{fig:visualaptitudecat}. 
...
\newtext{Scoring of all test results was done such that random guessing will lead to an expected value of 0; thus for a test consisting of multiple choice questions with $k$ possible answers and one correct answer each, the score is calculated as}
\begin{eqnarray}\label{eq.scoring}
\# \text{total correct answers } - 1/k \cdot \text{\#wrong answers}.
\end{eqnarray}

This allows us to compare each participant's score in light of how many problems were attempted as well as the number of correct responses. This scoring mechanism is also used on many standardized tests, such as the  SAT, ACT, and the battery of psychological tests which \newtext{parts} of this test are drawn from.
