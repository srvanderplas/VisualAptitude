...
<<ResultsScaledScores, echo=F, include=T, fig.cap="Test scores for lineups and visuospatial tests. As none of the participants scored at or below zero, we can conclude that there is little evidence of random guessing. \\label{fig:Scores}", fig.width=8, fig.height=3, out.width='.9\\textwidth'>>=
...
<<correlations, fig.cap="Scree plot of principle component analysis of performance on the different test batteries.", out.width=".45\\linewidth", fig.height=4.5, fig.width=4.5, comment="|", warning=F, message=F>>=
