...
\subsection{Scoring and Scaling of Answers}\label{sec:scaling}
...
ISU Students & \Sexpr{round(mean(ans.summary.unscaled$card_rot), 1)} (\Sexpr{round(sd(ans.summary.unscaled$card_rot), 1)}) 
             ...
             & \Sexpr{round(mean(ans.summary.unscaled$vis_search), 1)} (\Sexpr{round(sd(ans.summary.unscaled$vis_search), 1)})\\
...
\newtext{Applying the scaling protocol discussed in section \ref{sec:scaling}, we see that the ranges of scaled lineup and visuospatial test scores do not include zero; this indicates that we do not see random guessing from participants in any task. Figure \ref{fig:ScaledScores} shows the range of possible scores and the observed score distribution. } 
<<ResultsScaledScores, echo=F, include=T, fig.cap="Scaled test scores for lineups and visuospatial tests. As none of the participants scored at or below zero, we can conclude that there is little evidence of random guessing. \\label{fig:ScaledScores}", fig.width=8, fig.height=3, out.width='.9\\textwidth'>>=

score.summary <- melt(ans.summary, measure.vars=20:24, value.name = "Score", variable.name = "Test")
score.summary$Test <- factor(score.summary$Test, levels=c("lineup", "card_rot", "fig_class", "folding", "vis_search"), labels=c("Lineups", "Card Rotation", "Figure Classification", "Paper Folding", "Visual Search"))
qrange$sqrt.k <- sqrt(qrange$k-1)
qrange$Test <- factor(qrange$testtype, levels=c("lineup", "card_rot", "fig_class", "folding", "vis_search"), labels=c("Lineups", "Card Rotation", "Figure Classification", "Paper Folding", "Visual Search"))
qplot(data=score.summary, x=Test, y=Score, geom="boxplot") + 
  geom_rect(data=qrange, aes(xmin=as.numeric(Test)-.4, xmax=as.numeric(Test)+.4, ymin=-sqrt.k, ymax=sqrt.k, colour="red"), fill=NA, inherit.aes=F) + 
...
pca.cog.tests <- prcomp(ans.summary[,c("card_rot", "fig_class", "folding", "vis_search")], retx=T, scale=T)
