...
<<pca, fig.cap="Plots of the principle components with observations. Visual Search and Paper Folding strongly contribute to both PC1 and PC2, while Figure Classification and Paper Folding strongly contributes to PC3 and Card Rotation strongly contributes to PC4", comment="|", fig.height=4.5, fig.width=4.5, warning=F, message=F, out.width='.45\\textwidth'>>=
...
<<pca-reg, comment="|", warning=F, message=F>>=
...
library(xtable)
print(xtable(tmp, digits=c(1, 1, 0, 3, 3, 2, 3)), include.rownames=F)
@
