...
\newtext{
"Lineups" are often used to evaluate the effectiveness of different graphical displays \cite{kosara2010mechanical,wickham2010graphical,majumder2013validation}; as the police counterpart, lineups consist of several distractor plots and one target. 
...
This removes some of the subjectivity from user evaluations of display effectiveness, and the procedure is simple enough that it does not generally require familiarity with data-based graphics. 
}\newtext{
While previous research has examined the link between certain types of graphical perception and spatial skills, it is important to identify any additional visual skills required by the lineup task, as well as better understand demographic characteristics (math education, research experience, age, gender) which may differentially impact performance\cite{humanfactorslineups}. 
Exploring the participant characteristics which contribute to successful lineup evaluation allows us to better understand the lineup task and potential sources of variability in the data. 
}
