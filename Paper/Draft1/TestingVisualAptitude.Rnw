...
\section{Results}
\newtext{Results are based on an evaluation of \Sexpr{nrow(lineup.summary)} undergraduate students at Iowa State University. }
%We have tested students, and plan to increase sample size once the summer semester begins. 
\comment{No woulds, shoulds or coulds}
Scoring for all of the tests was calculated such that with random guessing, the expected score on a problem would be 0; thus, for a test with possible answers A, B, C, D, and E, the score would be calculated by subtracting $1/4*\text{\# wrong answers}$ from the total number of correct answers. 
\comment{re-phrasing the above sentence:}
\newtext{Scoring of all test results was done such that random guessing leads to an expected value of 0; thus for a test consisting of multiple choice questions with $k$ suggested answers with a single correct answer each, the score is calculated as}
\begin{eqnarray}\label{eq.scoring}
\# \text{total correct answers } - 1/(k-1) \cdot \# \text{wrong answers}.
\end{eqnarray}

This allows us to compare each participant's score in light of how many problems were attempted as well as the number of correct responses. \newtext{Combining accuracy and speed into a single number does not only make a comparison of test scores easier, } this scoring mechanism is also used on many standardized tests, such as the  SAT, ACT, and the battery of psychological tests which \newtext{parts} of this test are drawn from.
\comment{We need a few citations here. }
\newtext{Additionally, we have to ensure that the ranges and units of test scores are comparable. Assume $n$ questions with $k$ choices (including one correct answer) each. 
...
%The ability to combine both accuracy and number of questions attempted into a single number makes comparing test scores much easier. 
%In general, the formula for computing the score would be $$1/(n-1)*(\text{\# wrong answers}) + (\text{\# correct answers})$$ where $n$ is the total number of possible answers. 
...
\comment{Split the next sentence up into two paragraph: what is shown in each of the figures, what are the immediate conclusions? chuck out the gaming hours from figure \ref{fig:visualaptitudects}, that's a demograph, so move it to the other plot (I am aware that nine demographics are easier to display than ten, but content trumps). What happened to the arts skills? I seem to have a positive association now - why is my data different from yours?}
...
lineup.summary$variable <- factor(lineup.summary$variable, labels=c("Card Rotation", "Figure Classification", "Paper Folding", "Visual Search", "Log10 Video Game Hrs/Wk"))
lineup.summary$variable <- factor(lineup.summary$variable, levels=c("Card Rotation", "Figure Classification", "Paper Folding", "Visual Search", "Log10 Video Game Hrs/Wk"))
...
qplot(data=lineup.summary.categorical, x=value, y=lineup, geom="boxplot") + facet_wrap(~variable, scales="free_x", ncol=3) + xlab("Value") + ylab("Lineup Score (# Correct - 1/19 # Incorrect)") + theme_bw() + geom_point(aes(x=value, y=lineup), shape=1, size=3)
...
\newtext{All results and data shown here are done in accordance with IRB \# XXX?}
