...
\comment{OK, slow down! The intro is still a bit disconnected - I've tried to add the sentences that I think are missing in the first paragraph, but I'm not sure what story you want to tell. What would help is to write a sentence each to summarise a paragraph, and then work on connections between the paragraph. }
...
- why are we interested in evaluating which skills are necessary to evaluate graphical displays (this has been implicitly answered by saying that cognitive load was reduced, so we free up resources and mak graphics a better tool for more people.
