...
Table~\ref{tab:PCAvariance5} shows the importance of each principal component. \newtext{From the distribution of the variance components, we see that the lineup test  spans an additional dimension within the space of the four established tests.} From the rotation matrix (see Table~\ref{tab:PCArotation5}) we see that  
...
\newtext{The demographic variables are highy correlated, e.g.~there is a high correlation between STEM majors and taking calculus 1 -- only eight students are either not STEM, but have taken calculaus 1 or are STEM but have not taken the class. The situation between a statistics class and doing research is even more correlated. Only one student is doing research who has (not yet ?) taken a statistics course. A principal component of the five math/stats questions splits the variables into two areas: the first principal component is an average of math skills, calculus 1 and STEM, while the second principal component is an average of having taken a statistics class and doing research. We therefore decided to use sums of these variables to come up with a separate math and a stats score. Note, that the correlation between the math and the stats score is almost zero. }
...
@
\newtext{We fit a linear model of lineup scores in all of the demographic variables and the test scores from the visuo-spatial tests. Then we use a step-wise backwards selection based on AIC (Akaike Information Criterion - do we need a reference?) to select the best model. The result is shown in table~\ref{tab:m1}. Only two covariates stay in the model: PC1 and MATH, reflecting two dimensions of what affects lineup scores. We can think of PC1 as an innate visual ability, while the MATH score is a matter of both ability and training. 
}
<<modelresults, echo=FALSE, results='asis'>>=
library(xtable)
xtable(summary(m1), caption="Estimates and significances of a linear model of lineup scores.", label="tab:m1")
@
...
lineup.type.sum2$plot.type <- factor(lineup.type.sum2$plot.type, levels=c("boxplot", "jitter.boxplot", "violinplot", "bee.swarm.boxplot", "scatterplot", "density", "histogram", "qq.line", "qq.line.err", "qq.line.err.rot"))
...
<<lineup-types-scores,echo=F, fig.cap="Plot of scaled lineup scores by aptitude test scores", fig.width=6, fig.height=12, out.width=".65\\linewidth", fig.env="figure*", warning=F, message=F>>=
lineup.type.sum3 <- dcast(lineup.type.sum2, id + testtype ~ plot.type, value.var="pos.pts", fun.aggregate = sum)
...
qplot(data=lineup.type.sum3, 
  ...
  facet_grid(lineup.type~testtype, scales="free") + 
