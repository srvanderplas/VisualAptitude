...
This scaling gets slightly more complicated for scores which have two sub-groups, as with the Figure~Classification test. To get a single unified score with standard deviation, we completed the following calculations: 
\begin{eqnarray}
...
@
% \section{Lineup Performance: Accounting for ``Visual Intelligence''}
