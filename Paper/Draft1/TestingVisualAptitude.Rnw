...
<<lineup-types,echo=F, fig.cap="Plot of scaled scores for different types of lineups", fig.env="figure*", fig.width=6, fig.height=6, out.width=".9\\linewidth">>=
lineup.type.sum$plot.type <- factor(lineup.type.sum$plot.type, levels=c("boxplot", "jitter.boxplot", "violinplot", "tree.boxplot", "scatterplot", "density", "stacked.bar", "qq.line", "qq.line.error", "qq.line.error.rotated"))
qplot(data=lineup.type.sum, x=score, y=..scaled.., colour=factor(testnum), geom="density", cut=4) + 
  facet_wrap(~plot.type, ncol=5) + 
  xlab("Percent of Lineups Correct") + 
  ylab("Scaled Density") + 
  geom_vline(aes(xintercept=0), linetype=3) + 
  scale_colour_discrete("Lineup Test Number") + 
  scale_x_continuous(breaks=c(-.33, 0, .33, .67, 1)) +
  theme_bw() + theme(legend.position="bottom", legend.direction="horizontal", axis.text.x=element_text(angle=30))
@
Conclusion: stacked bar plots are awful. 
\end{document}
