...
all2mean <- rbind(
  ddply(subset(all2, !grepl("lineup", yvar)), .(xvar, yvar), summarize, label=paste0("Cor = ", round(cor(x,y), 3)), x=max(x), y=min(y)-1, hjust=1, vjust=0),
...
ggplot()+
  ...
  geom_text(data=all2mean, aes(x=x, y=y, label=label, hjust=hjust, vjust=vjust)) + 
...
qplot(x=0,  y=Var1, xend=value*importance, yend=Var1, geom="segment", data=rotation, arrow=arrow(length=unit(0.1, "cm"))) + facet_wrap(~Var2, ncol=4) + xlab("Contribution to each PC * % Variance in PC") + ggtitle("Lineup Types and Visual Aptitude") + ylab("") + theme_bw()
@
...
In addition, when lineup tasks with different goals are viewed separately, there is some indication that different tasks are associated with different visual abilities. 
